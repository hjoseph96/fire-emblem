...
public partial class AssetList : BaseScreen
{
	...
	void DrawTopBar(Rect position, BuildInfo buildReportToDisplay, BuildReportTool.FileFilterGroup fileFilterGroupToUse)
	{
		...
		GUILayout.BeginHorizontal(GUILayout.Height(11));
			...
			int viewOffset = _searchResults != null ? _searchViewOffset : assetListUsed.GetViewOffsetForDisplayedList(fileFilterGroupToUse);
	...
	void DrawAssetList(Rect position, BuildInfo buildReportToDisplay, AssetDependencies assetDependencies, BuildReportTool.AssetList list, BuildReportTool.FileFilterGroup filter, int length)
	{
		if (list == null)
		{
			return;
		...
		if (GUILayout.Button("     Asset Path", sortTypeAssetFullPathStyleName, BRT_BuildReportWindow.LayoutListHeight))
		{
		...
		{
			pressedSizeBeforeBuildSortBtn = DrawColumn(viewOffset, len, BuildReportTool.AssetList.SortType.SizeBeforeBuild, "Size Before Build   ", true, false,
		...
		{
			pressedRawSizeSortBtn = DrawColumn(viewOffset, len, BuildReportTool.AssetList.SortType.ImportedSizeOrRawSize, "Size In Build", true, false,
		...
		{
			pressedRawSizeSortBtn = DrawColumn(viewOffset, len, BuildReportTool.AssetList.SortType.RawSize, (IsShowingUnusedAssets ? "Raw Size" : "Size In Build"), true, false,
		...
		if (IsShowingUnusedAssets)
		{
			pressedImpSizeSortBtn = DrawColumn(viewOffset, len, BuildReportTool.AssetList.SortType.ImportedSize, "Imported Size   ", true, showScrollbarForImportedSize,
		...
		if (IsShowingUsedAssets)
		{
			pressedPercentSortBtn = DrawColumn(viewOffset, len, BuildReportTool.AssetList.SortType.PercentSize, "Percent   ", true, true,
				list, assetListToUse, (b) => {
	...
	bool DrawColumn(int sta, int end, BuildReportTool.AssetList.SortType columnType, string columnName, bool allowSort,
