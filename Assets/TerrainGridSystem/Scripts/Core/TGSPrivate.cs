...
namespace TGS {
    ...
    public partial class TerrainGridSystem : MonoBehaviour {
        ...
        public void Init() {
            ...
            canUseGeometryShaders = useGeometryShaders && SystemInfo.graphicsDeviceType != UnityEngine.Rendering.GraphicsDeviceType.Metal;
        ...
        void GenerateCellsMesh() {
            ...
            int meshGroups = (frontiersPointsCount / 65500) + 1;
            int meshIndex = -1;
            ...
            if (frontiersPointsCount == 0) {
                ...
                for (int k = 0; k < frontiersPointsCount; k += 65500) {
                    int max = Mathf.Min(frontiersPointsCount - k, 65500);
                    ++meshIndex;
        ...
        //	OLD VERSION: kept for now in case some users need to restore this behaviour	
        ...
        void DrawCellBorders() {
            ...
            for (int k = 0; k < cellMeshBorders.Length; k++) {
                ...
                Mesh mesh = new Mesh();
                if (_cellBorderThickness > 1 && !canUseGeometryShaders) {
        ...
        void DrawTerritoryFrontiers() {
            ...
            for (int t = 0; t < territoryMeshes.Count; t++) {
                ...
                for (int k = 0; k < tm.territoryMeshBorders.Length; k++) {
                    ...
                    Mesh mesh = new Mesh();
                    if (_territoryFrontiersThickness > 1 && !canUseGeometryShaders) {
        ...
        GameObject GenerateRegionSurface(Region region, int cacheIndex, Material material, Vector2 textureScale, Vector2 textureOffset, float textureRotation, bool rotateInLocalSpace) {
            ...
            try {
                ...
                for (int triBase = 0; triBase < triCount; triBase += 65500) {
                    ...
                    GameObject surf = Drawing.CreateSurface(surfName, meshPoints, triNew, material, rect, textureScale, textureOffset, textureRotation, rotateInLocalSpace, _sortingOrder, disposalManager);
        ...
        GameObject GenerateRegionSurfaceSimpleHex(Poly2Tri.Polygon poly, Region region, int cacheIndex, Material material, Vector2 textureScale, Vector2 textureOffset, float textureRotation, bool rotateInLocalSpace) {
            ...
            Rect rect = (canvasTexture != null && material != null && material.mainTexture == canvasTexture) ? canvasRect : region.rect2D;
            GameObject surf = Drawing.CreateSurface(cacheIndex.ToString(), meshPoints, hexIndices, material, rect, textureScale, textureOffset, textureRotation, rotateInLocalSpace, _sortingOrder, disposalManager);
            _lastVertexCount += 6;
            surf.transform.SetParent(surfacesLayer.transform, false);
            surfaces[cacheIndex] = surf;
            surf.transform.localPosition = Misc.Vector3zero;
        ...
        GameObject GenerateRegionSurfaceSimpleQuad(Poly2Tri.Polygon poly, Region region, int cacheIndex, Material material, Vector2 textureScale, Vector2 textureOffset, float textureRotation, bool rotateInLocalSpace) {
            ...
            Rect rect = (canvasTexture != null && material != null && material.mainTexture == canvasTexture) ? canvasRect : region.rect2D;
            GameObject surf = Drawing.CreateSurface(cacheIndex.ToString(), meshPoints, quadIndices, material, rect, textureScale, textureOffset, textureRotation, rotateInLocalSpace, _sortingOrder, disposalManager);
            _lastVertexCount += 4;
            surf.transform.SetParent(surfacesLayer.transform, false);
            surfaces[cacheIndex] = surf;
            surf.transform.localPosition = Misc.Vector3zero;
